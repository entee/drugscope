...
# echo "    Create and load tables RxNorm ... `/bin/date`" >> mysql.log 2>&1
# $MYSQL_HOME/bin/mysql -vvv -u $user -p$password -h$dbserver $db_name < Table_scripts_mysql_rxn.sql >> mysql.log 2>&1
# if [ $? -ne 0 ]; then ef=1; fi

# if [ $ef -ne 1 ]
# then
# echo "    Load Data RxNorm ... `/bin/date`" >> mysql.log 2>&1
# $MYSQL_HOME/bin/mysql -vvv -u $user -p$password -h$dbserver $db_name < Load_scripts_mysql_rxn_unix.sql >> mysql.log 2>&1
# if [ $? -ne 0 ]; then ef=1; fi
# fi

echo "    Create and load tables FDA ... `/bin/date`" >> mysql.log 2>&1
$MYSQL_HOME/bin/mysql -vvv -u $user -p$password -h$dbserver $db_name < Table_scripts_mysql_fda.sql >> mysql.log 2>&1
if [ $? -ne 0 ]; then ef=1; fi

if [ $ef -ne 1 ]
then
echo "    Load Data FDA ... `/bin/date`" >> mysql.log 2>&1
$MYSQL_HOME/bin/mysql -vvv -u $user -p$password -h$dbserver $db_name < Load_scripts_mysql_fda_unix.sql >> mysql.log 2>&1
if [ $? -ne 0 ]; then ef=1; fi
fi
